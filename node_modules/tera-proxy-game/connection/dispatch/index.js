...
class Dispatch extends EventEmitter {
	...
	// name|code, version, buf
	parse(name, version, buf) {
		return this.protocol.parse(name, version, buf)
	}

	// name|code, version, val
	serialize(name, version, val) {
		return this.protocol.write(name, version, val)
	}

	// name|code, version, val
	packetLength(name, version, val) {
		return this.protocol.length(name, version, val)
	}

	// name, version, str
	addDefinition(name, version, str) {
		const versions = TeraProtocol.defs.get(name)
		if(versions && versions.has(version)) throw Error(`Definition already exists: ${name}.${version}`)

		TeraProtocol.addDef(name, version, str)
			...
			if(hook.definitionVersion === 'raw')
				...
				try {
					const defVersion = hook.definitionVersion,
						event = this.protocol.parse(code, defVersion, data)
