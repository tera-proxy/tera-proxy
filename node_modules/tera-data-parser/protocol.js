...
class TeraProtocol {
	constructor(shareRevision) {
		...
		try {
			this.packetEnum = enumParser(fs.readFileSync(path.join(teraData, `map/protocol.${this.shareRevision}.map`), 'utf8'))
		}
		catch(e) {
			if(e.code === 'ENOENT') throw Error(`protocol.${this.shareRevision}.map not found`)
			e.message = `Error reading protocol.${this.shareRevision}.map: ${e.message}`
		}

		try {
			this.sysmsgEnum = enumParser(fs.readFileSync(
				path.join(teraData,`map/sysmsg.${this.sysmsgVersion || this.majorPatchVersion}.map`),'utf8'))
		}
		catch(e) {
			if(e.code === 'ENOENT') throw Error(`sysmsg.${this.sysmsgVersion || this.majorPatchVersion}.map not found`)
			e.message = `Error reading sysmsg.${this.sysmsgVersion || this.majorPatchVersion}.map: ${e.message}`
		}
