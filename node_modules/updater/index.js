...
class Updater {
	...
	async update(opts) {
		...
		if(opts.compat) {
			remoteManifest = manifestFromCaali(remoteManifest)

			// Compat for Bernkastel's mods
			if(!remoteManifest.data || remoteManifest.noHash && !Object.keys(remoteManifest.data).length) return false
		}
	...
	async updateSelf(dir, manifest, files, deleted) {
		...
		for(let [file, data] of files) {
			// Use index as temp filename to prevent duplicate files from causing errors
			const tmp = path.resolve(path.join(dir, '_update_', String(i++)))
	...
	async downloadFiles(manifest, files) {
		...
		for(const file of files)
			promises.add((async () => {
				...
				if(!manifest.noHash && crypto.createHash('sha256').update(data).digest('base64') !== manifest.data[file])
...
function manifestFromCaali(manifest) {
	...
	manifest = {
		noHash: manifest.no_hash_verification,
		data: manifest.files
	}
