		...
		Object.assign(this, {
			...
			customize: {
				...
				write(data, val) {
					if(val == null) data.buf.fill(0, data.pos, data.pos += 8)
						...
						data.dv.setUint8(data.pos, this.unk)
						data.dv.setUint8(data.pos + 1, this.skinColor)
						data.dv.setUint8(data.pos + 2, this.faceStyle)
						data.dv.setUint8(data.pos + 3, this.faceDecal)
						data.dv.setUint8(data.pos + 4, this.hairStyle)
						data.dv.setUint8(data.pos + 5, this.hairColor)
						data.dv.setUint8(data.pos + 6, this.voice)
						data.dv.setUint8(data.pos + 7, this.tattoos)
						data.pos += 8
					}
				},
				length: 8
			...
			string: {
				compileRead(compile, id) {
					...
					compile.str += `data.pos = ${pointer}\n`
					compile.str += `${id} = ''\n`
					compile.str += `for(let ${c}; ${c} = data.dv.getUint16(data.pos, true); data.pos += 2) ${id} += String.fromCharCode(${c})\n`
					compile.str += `data.pos = ${rtnPos}\n`
				},
			...
			bytes: {
				...
				compileWrite(compile, id) {
					...
					const main = compile.popState()
					compile.str += `if(!${id}.length) data.dv.setUint16(${ptrPos}, 0, true)\n`
					compile.str += `else {\n`
					compile.str += `data.dv.setUint16(${ptrPos}, data.pos, true)\n`
