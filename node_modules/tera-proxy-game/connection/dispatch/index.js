const EventEmitter = require('events'),
	...
	CompatWrapper = require('./wrapper-compat')
...
class Dispatch extends EventEmitter {
	...
	loadAll() {
		...
		log.info(`Loaded ${pluralize(count.true, 'mod')} in ${Date.now() - startTime}ms${
			count.false ? log.color('sred', ` (${count.false} failed)`) : ''
		}`)
	}
	...
	load(name, hotswapProxy) {
		...
		log.info(`Loading ${log.color(pkg.permissions ? 'smagenta' : 'stext', name)}${(() => {
			switch(pkg._compat) {
				case 1: return ` ${log.color('gray', '(legacy)')}`
				case 2: return ` ${log.color('gray', '(compat)')}`
				default: return ''
			}
		})()}`)

		try {
			const mod = new (pkg._compat === 2 ? CompatWrapper : Wrapper)(pkg, this)
			mod.init((pkg._compat === 2 ? compat.require : require)(this.modManager.resolve(name)), hotswapProxy)
