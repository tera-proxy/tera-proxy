...
class Updater {
	...
	async update(opts) {
		...
		if(opts.compat) {
			...
			if(!remoteManifest.data) return false
		}
		...
		const filesNew = diffNew(localManifest.data, remoteManifest.data),
			filesDeleted = remoteManifest.noDelete ? [] : diffDeleted(localManifest.data, remoteManifest.data)
...
function manifestFromCaali(manifest) {
	...
	manifest = {
		noDelete: manifest.no_hash_verification, // Compat for Bernkastel's mods
