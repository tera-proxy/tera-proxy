...
class ModManager {
	constructor(opts) {
		Object.assign(this, {
			...
			publicKeys: typeof opts.publicKeys === 'object' ? new Map(Object.entries(opts.publicKeys)) : new Map(),
	...
	preloadMods() {
		...
		const stats = new Stats()
		for(let [name, pkg] of this.packages)
			if(pkg.permissions?.includes('preload') || !this.autoUpdate)
				stats.update(this.preloadMod(name))
	...
	async loadPackage(_path) {
		...
		if(fs.lstatSync(_path).isDirectory()) {
			...
			try {
				...
				// Attempt to load manifest and verify signature
				/*	TODO: Verify hash of all files to prevent using a fake manifest in conjunction with non-updating mod.json.
					This is currently a minor issue as 'preload' permission is mainly to prevent misuse of RCE immediately after updates.
				*/
				try {
					const manifest = JSON.parse(fs.readFileSync(path.join(_path, 'manifest.json'), 'utf8'))

					if(manifest.signature && this.publicKeys.has(manifest.publicKey))
						if(manifest.noHash)
							log.warn(`"${baseName}" noHash cannot be used with signed manifest`)
						else if(!crypto.verify(
							null,
							Buffer.from(JSON.stringify(manifest.data)),
							crypto.createPublicKey({ key: Buffer.from(manifest.publicKey, 'base64'), type: 'spki', format: 'der' }),
							Buffer.from(manifest.signature, 'base64')
						))
							log.warn(`"${baseName}" signature is invalid`)
						else
							permissions = this.publicKeys.get(manifest.publicKey)
				}
				catch {}
			}
			catch(e) {
				// Toolbox package (module.json)
				try {
		...
		// Read-only permissions (signed packages only)
		/*	TODO: Remove or proxy mod.dispatch as that can allow a mod to easily rewrite its own package:
			mod.dispatch.modManager.packages.set(mod.name, Object.assign({}, mod.info, { permissions: ['malicious'] }))

			As the only permission is 'preload', this has little impact for the time being (only allows editing info in UI).
		*/
		Object.defineProperty(pkg, 'permissions', { enumerable: true, value: Object.freeze(permissions) })
	...
	preloadMod(name) {
		...
		try {
			const pkg = this.packages.get(name),
				mod = (pkg._compat === 2 ? compat.require : require)(this.resolve(name))

			if(pkg._compat !== 2 && typeof mod !== 'function') throw Error('Mod does not export a constructor')
			return true
		}
		catch(e) {
...
function overrideUpdateUrl(url) {
	...
	if(github) {
		...
		if(['tera-toolbox', 'tera-toolbox-mods'].includes(user.toLowerCase())) {
			switch(repo.toLowerCase()) {
				case 'fps-booster-2000':
				case 'library':
				case 'tera-guide':
					break
