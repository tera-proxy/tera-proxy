...
const kLog = Symbol('kLog'),
	...
	kToolboxLegacy = Symbol('kToolboxLegacy')
...
class CompatWrapper extends ModWrapper {
		...
		Object.defineProperty(this, 'require', {
			...
			value: new Proxy(Object.create(null), {
				get: (obj, key) => {
					...
					return this[kToolboxLegacy] || typeof mod === 'function' ? mod
						: !mod.requireInterface ? Object.assign({}, mod)
						: mod.requireInterface(mod.globalMod, mod.clientMod, mod.networkMod, this)
				},
			...
			try {
				if(this.settings._version !== settingsVersion) {
					this.settings = require(
						path.join(this.rootFolder, this.info.options.settingsMigrator || 'module_settings_migrator.js')
	...
	init(modConstructors) {
		...
		if(modConstructors.ClientMod) this[kLog].warn('GPK mod not supported in compat mode - some features may not work')
		if(modConstructors.NetworkMod) this._instance.networkMod = new modConstructors.NetworkMod(this)
		if(modConstructors.RequireInterface) modConstructors.requireInterface = modConstructors.RequireInterface
	}
	...
	[kCheckLogAbuse]() {
		...
		if(msg.includes('trying to use this module on an unsupported')) {
			this[kLog].warn('This mod contains anti-features aimed at degrading user experience for tera-proxy users'
				+ '\nPlease contact [ Pinkie Pie#7969 ] on Discord to report this issue')
			return this[kLogAbuse] = true
		}
	}
