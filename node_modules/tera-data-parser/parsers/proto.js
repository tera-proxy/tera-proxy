class ProtoCompiler {
	...
	write(type, id) {
		if(typeof type[0] === 'number') { // Fixed-length array
			...
			this.str += `if(${temp} == null) ${temp} = []\n`
			for(let i = 0; i < type[0]; i++) this.write(type[1], `${temp}[${i}]`)
		}
	...
	length(type, id) {
		if(typeof type[0] === 'number') { // Fixed-length array
			...
			const length = this.length(type[1], `${id}[0]`)
			...
			const temp = this.local()
			str += `let ${temp} = ${id}\n`
			str += `if(${temp} == null) ${temp} = []\n`
			str += length // Copy first array element that we used as a check earlier
			for(let i = 1; i < type[0]; i++) str += this.length(type[1], `${temp}[${i}]`)
			return str
		}
